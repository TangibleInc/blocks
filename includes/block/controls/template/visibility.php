...
return new class($conditions) {
  ...
  function &get_current_condition() {
    ...
    $conditions = &$this->stored_conditions[ $this->current_block_id ];

    if( ! $this->current_repeater ) return $conditions[ 'general' ];

    if( ! isset( $conditions[ 'repeater' ][ $this->current_repeater ]) ) {
      $conditions[ 'repeater' ][ $this->current_repeater ] = [];
    }
    
    return $conditions[ 'repeater' ][ $this->current_repeater ];
  }

  function start_repeater( $name ) {
    $this->current_repeater = $name;
  }

  function end_repeater() {
    $this->current_repeater = false;
  }
